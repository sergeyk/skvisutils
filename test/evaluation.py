...
from skpyutils import skutil, Table
from skvisutils import evaluation, Dataset

class TestEvaluationSynthetic(object):
  def __init__(self):
    self.d = Dataset(config, 'test_data2')
  ...
  def test_values(self):
    ...
    dets = det_gt.append_column('score', scores)
    ...
    ap = evaluation.compute_det_map(dets, det_gt, self.d.values)
    assert(ap==1)
    ap = evaluation.compute_det_map(dets_just_A, det_gt, self.d.values)
