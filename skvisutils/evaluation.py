...
def evaluate_per_class_detections(dataset, dets, dirname, force=False):
    ...
    for cls_ind in range(mpi.comm_rank, num_classes, mpi.comm_size):
        ...
        ap = compute_and_plot_pr(
            cls_dets, cls_gt, object_class, dirname, force)
        dist_aps[cls_ind] = ap
...
def evaluate_detections_whole(dataset, dets, dirname, force=False):
    ...
    if mpi.comm_rank == 0:
        ...
        template_filename = os.path.join(
            SUPPORT_DIR, 'dashboard_template.html')
        template = Template(filename=template_filename)
        filename = os.path.join(dirname, 'whole_dashboard.html')
        with open(filename, 'w') as f:
            f.write(template.render(support_dir=SUPPORT_DIR,
                    names=names, aps=aps_to_print, pngs=pngs))
    mpi.safebarrier()
        ...
        if plot:
            try:
                plot_filename = os.path.join(
                    dirname, 'pr_whole_{0}.png'.format(name))
...
def compute_and_plot_pr(dets, gt, name, dirname, force=False):
    ...
    """
    return compute_pr(dets, gt, name, dirname, force, plot=True)
...
def compute_cls_map(clses, gt):
    ...
    aps = []
    for col in gt.cols:
        ap, rec, prec = compute_cls_pr(
            clses.subset_arr(col), gt.subset_arr(col))
        aps.append(ap)
    return np.mean(aps)
...
def compute_cls_pr(confidences, gt):
    ...
    fp = not gt[ind]
    npos = gt.sum()
    return compute_rec_prec_ap(tp, fp, npos)
